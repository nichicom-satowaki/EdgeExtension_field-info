...
母子保健カテゴリ内には**37件のDTO**が登録されています。個別実行または「全部」ボタンで一括処理が可能です。

主なDTO例：
- 妊娠届出情報（A103PregnancyDto）
- 妊娠届出アンケート（A103PregnancyQuestionnaireDto）
- 母子健康手帳交付情報（A103IssueDto）
- 乳幼児健診結果（A103CheckupDetailDto）
- 母子保健訪問結果（A103VisitResultDetailDto）
- その他32件

※ 詳細なDTO一覧は [config.json](config.json) を参照してください。
...
- **状態フラグ**: `isProcessingDto` で同時実行を防止。
- **ページ遷移処理**: メンバーページからURL遷移で検索画面に戻る。ページリロード後、キューが自動的に復元・再開される。
...
- 母子保健カテゴリのDTOを5件から37件に拡充。
- 不要なコード削除によるメンテナンス性向上（約200行削減）。
- デバッグログの整理・簡潔化。
- ページ遷移処理をURL直接遷移に統一。
